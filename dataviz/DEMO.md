...
python3 ml/export_snapshot.py ml/data/dev.db dataviz/data/snapshot.json
...
```bash
while true; do python3 ml/export_snapshot.py ml/data/dev.db dataviz/data/snapshot.json >/dev/null; sleep 30; done &
```

## 2. 数据刷新流水线（换库或重播数据后按序执行）

```bash
python3      ml/gen_history.py   ml/data/dev.db --commit --reset   # 1 重播历史
python3      ml/check_signal.py                                    # 2 信号体检，四项须全过
.venv/bin/python ml/build_features.py                              # 3 特征面板
.venv/bin/python ml/train_forecast.py                              # 4 训练 + 评估报告
.venv/bin/python ml/predict.py --commit --prune                    # 5 推理回写 t_load_forecast
python3      ml/export_snapshot.py ml/data/dev.db dataviz/data/snapshot.json   # 6 导出快照
```

顺序不能乱：3 依赖 1 的产物，5 依赖 4 训出的模型与 `meta.json` 里的 is_peak 阈值，6 依赖 5 的回写。
...
顶栏「数据截止」会标成黄色提示这一点。答辩时服务端在跑，现场充电产生的订单就是今天的，这个数会自己填上。
**别在现场才发现，也别临时去改生成器。**
...
```bash
.venv/bin/python ml/predict.py --origin '2026-09-04 19:00:00' --commit --prune
```
