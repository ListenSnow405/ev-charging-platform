...
> 全部行数对应 **2026-09-14** 的仓库状态（09-10 的 3748 彻底删除、09-11 的收尾修复均已计入）。**被问到时按表直接翻，不要现场找。**

---

## 0. 三十秒定位（先说这段）

> 我负责 L5 数据端，做三件事：**一是造数据**——写历史数据生成器，给全组和模型提供燃料；**二是建模型**——特征工程加时序建模，预测未来 1、6、24 小时的充电负荷和空闲桩数，结果回写数据库；**三是做展示**——ECharts 运营大屏。另外我认领了一个扩展模块「碳减排与能源报告」，那个是全栈的，服务端 9 个命令字、管理端页面、大屏和对拍脚本都是我写的。
>
> 所以我的代码横跨 Python、C++ 服务端和 Qt 客户端三层，一共约 **9,500 行**。
...
| `ml/selftest.py` | 768 | 数据端自测：口径、守恒、穿越、边界 |
...
| `server/biz/ext_08_carbon_service.cpp` | 1877 | C++ 服务端 | 9 个 handler、数据库读写、报告生成与导出 |
| `server/biz/ext_08_carbon_calc_test.cpp` | 330 | C++ 测试 | 手算夹具 + 5000 例模糊测试，独立编译不连库 |
| `admin-client/ext_08_carbon_page.h/.cpp` | 1697 | Qt 客户端 | 碳排放报告页 + 新增因子对话框 |
| `common/protocol_ext.h` | 38 | 契约 | 命令字 3740–3748 |
| `common/error_code_ext.h` | 52 | 契约 | 错误码 6701–6707 |
...
| `scripts/smoke-carbon.py` | 787 | 测试 | 9 个命令字的协议冒烟测试 |
...
3. **可视化与扩展模块**。ECharts 大屏；以及我额外认领的碳减排模块，这个是全栈的，从建表、服务端 9 个命令字、Qt 管理端页面到大屏和对拍脚本。
