...
- `1` 有预测站点按拥堵度升序（同拥堵度再按距离），无预测站点（`congestion=-1`）排最后按距离升序 `[说明书]` 1.4「优先推荐低拥堵、高空闲率的充电站」
...
`1204/1205` 的 `amount` 单位**分**，按 `price(分/度) × kwhX100 / 100` 计算，不足一分钱部分**向下取整**（整数除法，非四舍五入）。`kwhX100` 由 L1 从设备上报转成整数提供，业务层禁止用 double 计算金额。
...
| 9005 | `CMD_DEV_START` | 开始充电（累计清零） | 服务端 → 设备 | `{pileCode}` |
| 9006 | `CMD_DEV_STOP` | 结束充电（停止累计） | 服务端 → 设备 | `{pileCode}` |

`9002` 的 `kwh`：**本次订单累计电量**（自收到 9005 开始、至 9006 停止的累计值），单位 kWh，是物理量（帧内为 double）。服务端收到后转成整数 `kwhX100`（×100，单位 0.01 kWh）供业务层结算取用；`power` 单位 kW。`9005/9006` 为服务端→设备推送（`seq=0`），由业务层通过 L1 的 `startCharging/stopCharging` 接口触发。
...
| 2026-09-08 | v1.3 | 新增 9005/9006 开始/结束充电；9002 kwh 定为累计电量；金额向下取整；1101 sortBy=1 无预测站排最后 | L1 | 待评审 |
