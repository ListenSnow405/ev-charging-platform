...
| 测试 | 人工测试流程与回归清单 | L4 / L5 | ✅ 数据端 [../ml/TESTING.md](../ml/TESTING.md)；一阶段客户端与全链路 [../第一阶段测试用例.xlsx](../第一阶段测试用例.xlsx)（9 张模块表 106 条用例 + 缺陷清单，94 条已实测，12 条 GUI 待人工执行）；二阶段 [../第二阶段测试用例.xlsx](../第二阶段测试用例.xlsx)（9 张模块表 139 条用例 + 缺陷 8 条，人工审核 11 条，由 `scripts/gen-testcases-phase2.py` 生成）|
