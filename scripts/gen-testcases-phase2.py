...
SHEETS.append(dict(
    ...
    cases=[
        ...
        ("P3-13", "切回本地目录不需改代码", "unset ECP_ODS_ROOT 后重跑 validation.py",
         "正常读本地 ODS，结论不变", p("[ODS] 本地　…/bigdata/ods；校验 14/14 通过"), "",
         "正常路径；代码只认路径不认介质"),
...
SHEETS.append(dict(
    ...
    cases=[
        ...
        ("P4-12", "类型转换失败不得静默变空", "ODS 一律按字符串读入",
         ...
         m("profiling/cleaning 均以 inferSchema=False 读入，转换与失败统计在 DWD 层完成。"), "",
         MAN + "；SOP 6.2 明文要求"),
    ]))
...
SHEETS.append(dict(
    ...
    purpose="验证 R001–R011 全部执行、全程零删除、校验 14 项全过、报告由产物程序生成",
    ...
    cases=[
        ("P5-01", "规则全部执行并留痕", "cleaning.py",
         ...
         p("规则执行 12 条（R001–R011 及分表执行），逐条记录命中数"), "", "正常路径"),
        ...
        ("P5-07", "校验全部通过方可进入下一阶段", "validation.py",
         "14 项全过；任一项不过以非零退出码结束",
         p("校验结果：14/14 通过　RESULT: PASS"), "", "正常路径；SOP 阶段 5"),
        ...
        ("P5-15", "质量报告由产物程序生成", "quality_report.py",
         ...
         p("质量报告已生成（140 行）；问题 11 条　规则执行 12 条　校验 14 项全过"), "",
         "正常路径；SOP 阶段 6；手写报告会与产物脱节"),
        ("P5-16", "校验不过必须拦住后续阶段", "使 validation 的任一断言失败",
         "以非零退出码结束，不允许带着失败的校验进入分析阶段",
         m("validation.py 在 failed>0 时以非零码返回，脚本内已实现；本轮 14/14 全过未触发该分支。"),
