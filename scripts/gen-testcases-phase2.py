...
并在备注里标明，不冒充自动化结果。未通过的照实写「不通过」，并在缺陷清单里留编号。

**日期有两个**：`DATE` 是首轮，`DATE2` 是增补/复跑那轮（用 p2 / m2）。
只有当轮真跑过的用例才换日期——表是哪天改的不重要，用例是哪天跑的才重要。

用例在表里**按编号排序**，源码里新增用例直接追加到列表末尾即可。
...
#  本轮（DATE2）真跑过的用例用 p2/m2，没重跑的一律留在 DATE——
#  日期跟着「哪天跑的」走，不跟着「哪天改的表」走，否则复跑记录就失去意义。
def p2(detail: str) -> str:
    return f"{OK2}\n实测：{detail}"


def m2(detail: str) -> str:
    return f"{OK2}\n实测：与预期一致（人工审核）。{detail}"


def ng2(detail: str) -> str:
    """未通过：照实记，不修饰；须在缺陷清单里有对应编号。"""
    return f"{NG2}\n实测：{detail}"
...
SHEETS.append(dict(
    ...
    cases=[
        ...
        ("P3-13", "切回本地目录不需改代码", "unset ECP_ODS_ROOT 后重跑 validation.py",
         "正常读本地 ODS，结论不变", p("[ODS] 本地　…/bigdata/ods；校验 82/82 通过"), "",
         "正常路径；代码只认路径不认介质"),
...
SHEETS.append(dict(
    ...
    purpose="验证 R001–R011 全部执行、全程零删除、校验 82 项全过、Schema 契约与主外键/范围/时序逐表断言、报告由产物程序生成",
    ...
    cases=[
        ("P5-01", "规则全部执行并留痕", "cleaning.py",
         ...
         p("规则执行 17 条（R001–R011 及分表执行），逐条记录命中数"), "", "正常路径"),
        ...
        ("P5-07", "校验全部通过方可进入下一阶段", "validation.py",
         "82 项全过；任一项不过以非零退出码结束",
         p("校验结果：82/82 通过　RESULT: PASS"), "", "正常路径；SOP 阶段 5"),
        ...
        ("P5-15", "质量报告由产物程序生成", "quality_report.py",
         ...
         p("质量报告已生成（458 行）；问题 11 条　规则执行 17 条　校验 82 项全过"), "",
         "正常路径；SOP 阶段 6；手写报告会与产物脱节"),
        ("P5-25", "Excel 副本可直接打开且不乱码", "归档包内的两个 .xlsx",
         "中文正常显示；金额电量为数值可直接求和；手机号/订单号仍为文本；空值留空",
         p("ODS 14 表 / DWD 11 表各一册；首两张为「说明」「表清单」，其余按业务顺序；"
           "实测 amount=12289 为数值、phone/order_no 为文本、avatar 空单元格"), "",
         "回归；评审意见⑦。CSV 为无 BOM UTF-8，中文 Windows 的 Excel 按 GBK 解会乱码；"
         "不给 CSV 加 BOM 是因为 ODS 那份是 Spark 输入，BOM 会混进首列列名"),
        ("P5-20", "全表主外键完整", "11 张 DWD 表的主键与 12 条外键",
         "主键非空唯一；外键全部命中维表（可空外键的 NULL 不算违规）",
         p("11 表主键全过；7 张有外键的表共 12 条关系全部命中"), "",
         "回归；评审意见②：原先只查订单表"),
        ("P5-21", "取值范围符合源库 CHECK", "45 条范围规则",
         "状态枚举、金额非负、评分 1–5、峰平谷求和等于总量等逐条满足",
         p("11 表 45 条规则全部满足；含 carbon_daily 峰平谷+未分配=总量的等式校验"), "",
         "边界值；约束在 SQLite 里生效，数据进 ODS 后无人守，DWD 必须自查"),
        ("P5-22", "时间连续性无缺口", "dwd_order 按日、dwd_carbon_daily 按站按日",
         "按业务频率建完整时间索引，缺失点数为 0",
         p("订单 60 天 60 点；碳排放 60 天 × 6 站 = 360 点，均无缺口"), "",
         "正常路径；SOP 6.8"),
        ("P5-23", "关键分布前后完全一致", "amount / kwh_x100 / price 与状态分布",
         "计数、合计、极值、四分位、状态构成逐项相等",
         p("三个字段的统计量与四分位逐项相等；状态分布 已结算 6477 / 已取消 1815 一致"), "",
         "正常路径；SOP 7.2，零删除零改值下标准定到最严"),
        ("P5-24", "报告结论不得写死", "quality_report.py 六维结论与遗留问题",
         "数字与条目均由产物现读，条件不成立的条目不出现",
         p("遗留问题 5 条全部由实测触发；待核条目现读 pending 样例；"
           "顺带修正原文案「8292 笔订单逐笔吻合」——实际参与校验的是 6477 笔已结算单"), "",
         "回归；评审意见⑤：硬编码换数据集会说假话"),
        ("P5-18", "DWD 符合 Schema 契约", "dwd_schema.py 声明的列序/类型/可空",
         "11 张表 132 列逐列一致；非空列无 NULL；列序不随实现漂移",
         p("11 表各 3 条断言全过；契约落地当场抓出 avatar/output_path 与 R001 的矛盾并订正"), "",
         "回归；评审意见①：缺数据字典与列序契约"),
        ("P5-19", "合理缺失的判定可追溯", "01_profile.json 合理缺失登记",
         "9 条登记含实测缺失率与理由，字段消失或已无缺失会标注「登记已过期」",
         p("9 条全部落盘；报告 3.1 节渲染成表"), "",
         "回归；评审意见③：理由原先只在运行时打印"),
        ...
        ("P5-16", "校验不过必须拦住后续阶段", "使 validation 的任一断言失败",
         ...
         m("validation.py 在 failed>0 时以非零码返回，脚本内已实现；本轮 82/82 全过未触发该分支。"),
...
SHEETS.append(dict(
    ...
    cases=[
        ("P6-01", "分析维度数量达标", "bigdata/analysis/_summary.json",
         "维度 ≥ 8",
         p2("dimension_count = 15（要求 ≥ 8）；D16/D17 智能运营建议并入后自动计数，无需手改"), "",
         "正常路径；[二阶段] 明文要求"),
        ("P6-02", "对比分析组数达标", "同上", "对比 ≥ 2 组",
         p2("comparison_count = 3（C1 快慢充 / C2 工作日周末 / C3 站点对标）"), "",
         "正常路径；[二阶段] 明文要求"),
        ...
        ("P6-10", "结果同时落 MySQL 与 JSON 快照", "MySQL 结果表 + bigdata/analysis/*.json",
         "两侧维度齐全且可被 API 读到",
         p2("MySQL 20 张结果表；/api/dimensions 返回 20 份（15 维度 + 4 对比表 + d12 预测表）"), "",
         "正常路径"),
        ("P6-12", "智能运营建议单独成表", "bigdata/spark/advice.py → D16 / D17",
         "预测结果须落到运营动作，用例见「10 智能运营建议」表",
         p2("d16_dispatch_advice 2 行、d17_alert_actions 12 行，已计入 dimension_count"), "",
         "正常路径；详见 10 表"),
...
SHEETS.append(dict(
    ...
    purpose="验证六页面板全部渲染出真实数据、DataV 组件生效、图表形态不单一",
    precond="① API 已启动；② cd bigdata/web && npm run build && npm run preview；"
            "③ 执行 .venv-phase2/bin/python scripts/smoke-screen-phase2.py [--url]。"
            "4173 被占用时 vite 会自动改用 4174，须把实际地址传给冒烟脚本，否则测的是别的服务",
    cases=[
        ("P8-01", "页面无全局错误", "geckodriver 打开 http://127.0.0.1:4173/",
         "无 .global-err 提示", p2("页面正常加载"), "", "正常路径"),
        ("P8-02", "分页标签渲染", "读取 nav.tabs",
         "六页标签齐全",
         p2("6 页：['1运营总览','2站点与设备','3对比分析','4负荷预测','5碳排放','6智能运营建议']"), "",
         "正常路径"),
        ("P8-03", "第 1 页运营总览渲染", "切到第 1 页", "面板齐全且 canvas 非空",
         p2("5 个面板　3 个非空 canvas"), "", "正常路径"),
        ("P8-04", "第 2 页站点与设备渲染", "切到第 2 页", "同上", p2("4 个面板　4 个非空 canvas"), "", "正常路径"),
        ("P8-05", "第 3 页对比分析渲染", "切到第 3 页", "同上", p2("4 个面板　4 个非空 canvas"), "", "正常路径"),
        ("P8-06", "第 4 页负荷预测渲染", "切到第 4 页",
         "D12 图与明细表渲染；MLlib 未跑时「暂无数据」属预期",
         p2("2 个面板　1 个非空 canvas"), "", "边界值；另一格是表格不产生 canvas"),
        ("P8-07", "第 5 页碳排放渲染", "切到第 5 页", "同上", p2("3 个面板　1 个非空 canvas"), "",
         "边界值；另两格为表格"),
        ("P8-20", "第 6 页智能运营建议渲染", "切到第 6 页",
         "值守/检修热力图 + 分流建议表 + 预警处置表均渲染",
         p2("3 个面板　1 个非空 canvas（另两格为表格）"), "",
         "正常路径；数据侧用例见 10 表"),
        ("P8-08", "面板数覆盖全部维度", "统计全部页面的面板总数",
         "下限式断言 ≥ 14，不写死会随开发增长的数",
         p2("实际 21 个面板（16 图表 + 5 表格）；本轮新增第 6 页 3 块后断言依旧成立，未再失效"),
         "BUG-P2-001",
         "回归用例；该用例发现断言写死 17 已过期，见缺陷清单。"
         "改成下限式后经历 D16/D17 两次扩页仍未误报，修法成立"),
        ("P8-09", "ECharts 真的画出像素", "统计非空 canvas 面积",
         "canvas 存在还不够，尺寸为 0 也「存在」", p2("当前页 1 个非空 canvas；KPI 7 块画布最少 286 个着色像素"), "",
         "边界值；只查存在性会漏掉空白图"),
        ("P8-10", "DataV 边框组件生效", "统计 BorderBox 元素",
         "使用 DataV 开源组件而非自绘边框", p2("当前页 BorderBox13 × 3"), "",
         "正常路径；[二阶段] 要求用 DataV"),
        ("P8-11", "DataV 数字翻牌生效", "统计 DigitalFlop", "顶部 KPI 使用翻牌组件",
         p2("DigitalFlop × 6"), "", "正常路径"),
        ("P8-12", "DataV 胶囊图与活动环图生效", "第 1 页",
         "两类 DataV 图表均渲染", p2("Capsule × 1　ActiveRing × 1"), "", "正常路径"),
        ("P8-13", "无页面级 JS 错误", "收集 window error", "无", p2("无"), "", "异常路径"),
        ("P8-14", "截图可产出", "geckodriver 截图",
         "产出非空 PNG，供答辩材料使用", p2("screenshot.png 412 KB"), "", "正常路径"),
        ("P8-15", "图表形态不得单一", "统计图表种类",
         "≥ 5 类不同形态",
         m2("实际 10 类：折线面积 / 柱 / 横向条形 / 堆叠柱 / 分组柱 / 环图 / 仪表盘 / 雷达 / 散点 / 热力图，"
            "另加 5 张滚动表格（D12 预测明细、D13、D14、D16、D17）。热力图为第 6 页新增。"),
         "", MAN + "；[二阶段] 明文要求"),
        ...
        ("P8-18", "生产构建可用", "npm run build",
         "构建成功并可由 preview 提供服务", p2("578 modules transformed；dist 产出；preview 200"), "",
...
SHEETS.append(dict(
    name="10 智能运营建议",
    module="第二阶段 · 智能运营建议（T9）",
    feature="把 MLlib 的 1h / 6h / 24h 预测翻译成运营动作：D16 站点分流建议、D17 预警处置清单（含模型可信度）",
    purpose="验证建议全部由已有产物派生、阈值与两个 Qt 客户端一致、可信度有实测依据、重复执行结果一致",
    precond="① d12_load_forecast.json 与 bigdata/mllib/eval.json 已产出；"
            "② 执行 .venv-phase2/bin/python bigdata/spark/advice.py",
    cases=[
        ("P10-01", "只用已有产物派生，不重训模型、不直连业务库", "advice.py 源码与运行日志",
         "输入限于 d12 预测、ODS 两张维表与 eval.json；不得出现 charging.db 连接",
         p2("数据源 d12_load_forecast.json / t_station.csv / t_pile.csv / eval.json；"
            "全文件无 sqlite 连接，charging.db 仅出现在说明该约束的注释里"), "",
         "正常路径；CLAUDE.md 5.2 第 10 条"),
        ("P10-02", "拥堵度口径与 predict.py 同源", "congestion = 1 − idle_pile / pile_total",
         "沿用预测输出的字段直接换算，不另立口径",
         p2("龙岗大运 idle_pile 2 / pile_total 4 → congestion_pct 50.0"), "",
         "正常路径；另立口径会让大屏与客户端对同一站给出两种拥堵度"),
        ("P10-03", "分流阈值与两个 Qt 客户端一致", "三处阈值常量比对",
         "0.8，与管理端预警、用户端提示同一口径",
         p2("advice.py BUSY = 0.8；user-client/main_window.cpp:52 CONGESTION_BUSY_THRESHOLD = 0.8；"
            "admin-client/forecast_page.cpp:29 CONGESTION_WARNING_THRESHOLD = 0.8"), "",
         "正常路径；三处不一致会让同一时刻的建议互相矛盾"),
        ("P10-04", "无站点达到分流阈值时不得给空表", "本轮预测最高拥堵 50% < 80%",
         "退到「引导」级（拥堵排前二且 ≥ 40%），并在建议文本里说明",
         p2("d16 共 2 行，advice_level 全为「引导」；最高 from_congestion_pct 50.0，未触发 80% 分流档"), "",
         "边界值；空表会被误读成功能没做，硬凑「拥堵」又是假数据"),
        ("P10-05", "候选站必须确实更空闲", "候选条件 拥堵 < 50% 且预测空闲 ≥ 2 桩",
         "推荐去向的拥堵度低于来源站，且有足够空闲桩",
         p2("南山科技园 50% → 深圳湾公园 25%/空闲 3、宝安中心 25%/空闲 3"), "",
         "正常路径；不设下限会把人引到同样满的站"),
        ("P10-06", "推荐按距离排序并给出公里数", "站点经纬度 haversine 距离",
         "近的排前面，建议文本含距离",
         p2("南山科技园 → 4.0km / 6.7km；龙岗大运 → 22.3km / 23.3km，均为近者在前"), "",
         "正常路径"),
        ("P10-07", "预警清单覆盖站点 × horizon", "d17_alert_actions",
         "6 站 × 1/6/24h 中触发预警的组合逐条出行",
         p2("12 行：高峰 6 / 低谷 6；按 horizon 为 1h 1 行、6h 5 行、24h 6 行"), "",
         "正常路径"),
        ("P10-08", "可信度来自实测增益，不是人为设定", "eval.json 的 gain_vs_b_pct",
         "两个目标都跑赢基线→高，赢一个→中，都没赢→低",
         p2("1h「负荷 +1.0%、并发 +7.9%（2/2 优于基线）」→ 高；24h 1/2 → 中；6h 0/2 → 低"), "",
         "正常路径；与 P9-07「结果如实报告」同源——跑输的 horizon 照样出建议，但标低可信度"),
        ("P10-09", "故障桩进入处置动作", "t_pile status=2 的站点",
         "有故障桩的站点给出抢修 / 远程重启动作",
         p2("龙岗大运 fault_piles=1 → 「高峰前完成抢修 / 远程重启（该站 1 台故障桩）」共 4 行"), "",
         "正常路径；[说明书] 远程重启电桩"),
        ("P10-10", "重复执行结果一致", "连续执行 advice.py 两次",
         "两张表 DROP + CREATE 重建，行内容完全相同",
         p2("两次 rows 逐字段相同，仅 generated_at 不同"), "", "正常路径；幂等"),
        ("P10-11", "结果同时落 JSON 与 MySQL 且 API 可取", "MySQL 结果表 + /api/dimension/*",
         "两侧行数一致，接口 code=0",
         p2("MySQL d16 2 行 / d17 12 行，与 JSON 一致；两个接口均 code=0"), "", "正常路径"),
        ("P10-12", "新增维度须自动计入 _summary", "bigdata/analysis/_summary.json",
         "dimension_count 由脚本更新，不靠手改",
         p2("dimension_count = 15（含 D16/D17），comparison_count = 3"), "",
         "回归；rebase 后曾出现 _summary 与结果文件对不上（见提交 20001bb）"),
        ("P10-13", "建议页在大屏渲染", "大屏第 6 页",
         "热力图 + 两张表格均出数", p2("第 6 页「6智能运营建议」3 个面板　1 个非空 canvas"), "",
         "正常路径；渲染侧断言见 08 表 P8-20"),
    ]))

SHEETS.append(dict(
    name="11 结项数据集归档包",
    module="第二阶段 · 结项数据集归档（L5）",
    feature="《数据清洗基本流程-操作SOP》第 8 节归档清单：原始只读快照、规则、脚本、执行日志、待核记录、清洗结果、质量报告",
    purpose="验证一条命令即可导出可直接提交的数据集压缩包，且包内口径、链接与指纹全部自洽",
    precond="六阶段产物齐全；执行 .venv-phase2/bin/python scripts/pack-dataset-phase2.py",
    cases=[
        ("P11-01", "一条命令导出压缩包", "pack-dataset-phase2.py",
         "直接产出 export/第二阶段数据清洗数据集.zip，无需人工拼装",
         p2("2.46 MB，解包后 85 个文件（含清单自身）；2026-09-16 16:44 一次跑通"), "",
         "正常路径"),
        ("P11-02", "归档内容对齐 SOP 第 8 节", "解包后目录",
         "六阶段目录 00–06 + 脚本 + 打包清单齐全",
         p2("00_原始层_ODS / 01_探查与质量评估 / 02_清洗规则 / 03_清洗结果_DWD / "
            "04_执行日志与待核清单 / 05_清洗校验 / 06_质量报告 / 脚本 / _打包清单.json"), "",
         "正常路径；按编号读下来就是完整清洗过程"),
        ("P11-03", "原始层随包归档且指纹可追溯", "00_原始层_ODS/_manifest.json",
         "源库指纹与打包清单一致，可回溯到具体的业务库快照",
         p2("源库指纹 2eff8f6c1537b4eb；14 表 CSV + manifest + HDFS 落地留痕 _hdfs_deploy.json"), "",
         "正常路径；CLAUDE.md 5.2 第 6 条"),
        ("P11-04", "DWD 同时给 parquet 与可读副本", "03_清洗结果_DWD",
         "parquet 为流水线真实产物，CSV 与 xlsx 为等价可读副本",
         p2("11 表 parquet + 11 份 CSV + DWD清洗结果.xlsx；dwd_order.csv 8292 行与 DWD 一致"), "",
         "正常路径；评阅人不装任何库也能核对"),
        ("P11-05", "CSV 时间不得平移 8 小时", "parquet 内按规范存 UTC",
         "由 Spark 按会话时区导出，与同行的 order_hour 自洽",
         p2("dwd_order.csv 8292 行逐行比对，reserve_time 的小时与 order_hour 不符 0 行；"
            "样例 2026-07-07 11:21:00 / order_hour=11"), "",
         "回归用例；用 pyarrow 直读会让 11:21 变成 03:21，而 order_hour 仍是 11"),
        ("P11-06", "整数列含空值不得被提升为浮点", "取消单的时长类字段",
         "空值写成空，整数不出现 48.0 这种写法",
         p2("start_time / end_time / settle_time / charge_minutes / wait_minutes / unit_price_fen "
            "各 1815 个空值；全表无小数写法"), "",
         "边界值；CLAUDE.md 5.2 第 7 条；走 pandas 会被提升成 float"),
        ("P11-07", "Excel 副本随包产出", "ODS原始数据.xlsx / DWD清洗结果.xlsx",
         "两册均在包内，首两张为「说明」「表清单」",
         p2("ODS 14 表 / DWD 11 表，两册齐全"), "", "正常路径；内容口径见 05 表 P5-25"),
        ("P11-08", "只读位在包内恢复为可写", "ODS 源文件为 444",
         "包内为 644，解压后可读可复制；只读性由指纹与质量报告证明，不靠权限位",
         p2("包内 t_order.csv 为 -rw-r--r--，源目录仍为 444"), "",
         "边界值；直接带走 444 会让评阅人连解压后的目录都删不掉"),
        ("P11-09", "Spark 校验碎片剔除、_SUCCESS 保留", "parquet 目录",
         ".crc 是噪音须删，_SUCCESS 标记写入完整须留",
         p2("解包后 .crc 0 个，_SUCCESS 11 个"), "", "正常路径"),
        ("P11-10", "包内相对链接不得断", "六阶段分目录后同级路径全部失效",
         "只改包内副本把链接指到新位置，仓库原件不动",
         p2("包内 markdown 相对链接 7 条，断链 0；仓库原件未被修改"), "",
         "正常路径；脚本内以 assert 守住「文档改了这里要跟着改」"),
        ("P11-11", "打包清单逐文件指纹可复算", "_打包清单.json",
         "每个文件的字节数与 sha256 均可独立复算",
         p2("84 条逐条复算，不一致 0；清单自身不登记，故磁盘 85 − 登记 84 = 1"), "",
         "正常路径；自己无法登记自己的指纹"),
        ("P11-12", "README 必须登记进清单", "清单由扫描暂存目录生成",
         "README 先写后扫，否则漏登",
         p2("README.md 在清单内；脚本以 assert 守住写入顺序"), "",
         "回归用例；上一版曾漏登（2026-09-15 评审指出）"),
        ("P11-13", "压缩包完整性", "unzip -t",
         "全部条目可解压，无 CRC 错误", p2("No errors detected in compressed data（108 个条目）"), "",
         "正常路径"),
        ("P11-14", "重复打包内容一致", "连续执行两次并逐文件比对指纹",
         "数据文件逐字节一致；只允许含时间戳的文件不同",
         p2("81/84 文件指纹相同；不同的 3 个为 README.md（含打包时间）与两册 xlsx——"
            "后者拆包后仅 docProps/core.xml 一个零件不同，即 Excel 内置创建时间，数据部分完全一致"), "",
         "正常路径；幂等的边界说清楚，比笼统说「完全一致」可信"),
        ("P11-15", "包内不得含版本库占位文件", "解包后查隐藏文件",
         "归档包面向评阅，不应出现 .gitkeep 这类仓库内部文件",
         ng2("00_原始层_ODS/.gitkeep 随 ODS 目录整体复制进包（1 个）。"
             "不影响数据正确性，但该文件正是 BUG-P2-002 里让本地/HDFS 文件数对不上的那个"),
         "BUG-P2-009", "缺陷用例；修复方式见缺陷清单，需重新打包一次"),
    ]))
...
DEFECTS = [
    ...
    ("BUG-P2-009", "P11-15", "结项数据集压缩包带入仓库占位文件 .gitkeep",
     "scripts/pack-dataset-phase2.py",
     "ODS 目录整体 copytree 进包，把只为让 git 留住空目录而存在的 .gitkeep 一并带走。"
     "评阅人解包后会在原始层看到一个与数据无关的隐藏文件；"
     "同一个文件此前已在 BUG-P2-002 里造成过本地与 HDFS 的文件数对不上",
     "轻（不影响数据正确性，属交付物整洁度）", "待修复",
     "拟在 copy() 增加忽略规则（.gitkeep 等仓库占位文件），改后需重新执行一次打包，"
     "并由 P11-11 的指纹复算与 P11-15 回归确认"),
...
def build() -> None:
    ...
    total = 0
    for sh in SHEETS:
        #  按编号出表：后补的用例在源码里追加即可，不必手工插到中间——
        #  源码顺序和表格顺序由这一行对齐，避免表里出现 P5-25 夹在 P5-15 后面。
        sh["cases"].sort(key=lambda c: int(c[0].split("-")[1]))
        ...
        style_meta(ws, 3, "编制人", AUTHOR, "编制时间", EDIT_TIME)
    ...
    ws.cell(3, 2, f"{AUTHOR}　编制时间 {EDIT_TIME}")
    for r in (1, 2, 3):
    ...
    failed = [c[0] for sh in SHEETS for c in sh["cases"] if str(c[4]).startswith(NG2)]
    open_bugs = [d[0] for d in DEFECTS if d[6] != "已修复并回归"]
    print(f"已生成 {OUT.name}")
    print(f"  {len(SHEETS)} 张模块表 + 缺陷清单　用例 {total} 条　缺陷 {len(DEFECTS)} 条")
    print(f"  其中人工审核项 {manual} 条（实际结果按约定记为等于预期）")
    print(f"  未通过 {len(failed)} 条 {failed or ''}　未闭环缺陷 {len(open_bugs)} 条 {open_bugs or ''}")
