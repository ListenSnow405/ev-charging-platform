...
| 充电量分桶 | D5 | **渐变柱** |
| 设备事件时序 | D8 | **散点**，点径随事件数 |
| MLlib 负荷预测 1/6/24h | D12 | **分组柱**（三个 horizon 同框） |
| 预测明细（空闲桩与高峰） | D12 | DataV **滚动表格** |
| 分站碳排放汇总 | D13 | DataV **滚动表格**（含条形背景单元格） |
| 最近 15 日碳排放明细 | D14 | DataV **滚动表格** |

共 **18 个面板**（15 个图表 + 3 张表格）/ **9 类图表形态**，对应「图表格式不要过于单一」的要求。

> ⚠ **改 `pages.js` 加减面板后，这张表和 `scripts/smoke-screen-phase2.py` 都要跟着看一眼。**
> T7 加「负荷预测」页时这两处都没更新——冒烟里写死的 `面板总数 == 17` 从那次提交起
> 一直是失败的，直到 09-15 补测试用例重跑才发现（BUG-P2-001）。
> 冒烟的断言已改成下限式，不再写死总数；这张表仍需手工维护。
