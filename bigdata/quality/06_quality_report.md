...
| R002/R003·t_pile | 维表类型转换（失败进待核） | 列数=7　失败行=0　表行数=24 |
...
**类型转换失败对账（R002/R003）**：`t_order` 0 行、`t_pile` 0 行、`t_station` 0 行、`t_pile_log` 0 行，合计 0 行。四张表各有一份待核清单，**空清单也落文件**——「查过且为空」与「没查」必须能区分。

> 全程**零删除**。SOP 第 5 节把删除列为最后手段，本轮发现的问题要么可标记（R006）、要么属正常业务（R007）、要么是分析范围问题而非数据错误（R009），都不需要动行。

## 5. 清洗前后对比与校验

阶段 5 共 20 项检查，失败 0 项。
...
| t_pile 转换失败清单已落地且为空（R002/R003） | **PASS** | type_cast_failed_pile.json 行数 0 |
| t_pile 转换列非空数无漂移 | **PASS** | 7 列逐列一致 |
| t_station 转换失败清单已落地且为空（R002/R003） | **PASS** | type_cast_failed_station.json 行数 0 |
| t_station 转换列非空数无漂移 | **PASS** | 5 列逐列一致 |
| t_pile_log 转换失败清单已落地且为空（R002/R003） | **PASS** | type_cast_failed_pile_log.json 行数 0 |
| t_pile_log 转换列非空数无漂移 | **PASS** | 3 列逐列一致 |
...
| 5 | 数据滞后 10 天 | 合成数据集的固有属性 | 无规则可施；答辩时主动说明数据为 `ml/gen_history.py` 合成 |
...
| 探查执行 | 2026-09-15 09:32:54 |
| 清洗执行 | 2026-09-15 11:33:12 |
| 校验执行 | 2026-09-15 11:37:29 |
| 代码版本 | `aa54b09` |
